...
log() {
    local log_file=$PATH_TO_FOLDER/script.log
...
curl -L -o "$PATH_TO_FOLDER/development_$build_id.zip" "$download_url"
...
unzip -o $PATH_TO_FOLDER/development_$build_id.zip

# Upload ipa file to the BrowserStack

id=$(curl -u "$BROWSERSTACK_USERNAME:$BROWSERSTACK_ACCESS_TOKEN" \
    -X POST "$BROWSERSTACK_API" \
    -F "file=$PATH_TO_IPA_FILE")

id_output=$(echo $id | jq -r '.app_url')
...
log "Upload ID: $id_output"

echo $id_output > $PATH_TO_ID
